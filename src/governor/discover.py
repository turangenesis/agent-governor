...
The "save it and replay without LLMs" trick (the smart part):
  - `python -m governor.discover`          -> runs the REAL search once, caches to discovered_candidates.json
  - `python -m governor.discover --show`   -> loads the cache, no network, deterministic (stage-safe)
