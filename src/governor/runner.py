...
Headless check:  python -m governor.runner
"""
...
class Orchestrator:
    def __init__(self, mode: str = "governor", zero_mode: str = "stub",
                 ...
                 source: str = "labeled", agent_mode: str = "flow",
                 search_fn=None, llm=None, replay_traces=None):
        self.mode = mode
        ...
        # agent_mode: how source="agents" workers run their territory.
        #   "flow" = the fixed deterministic LangGraph flow (offline/no-key safe).
        #   "loop" = the autonomous LLM tool-use loop, with a decision trace; falls back to
        #            "flow" when no tool-calling LLM is available (still runs with no key).
        self.agent_mode = agent_mode
        self.search_fn = search_fn    # optional offline search injection (tests/replay)
        # replay_traces: {territory_key -> persisted decision trace}. When set (loop mode), each
        # agent REPLAYS its recorded trace via replay_trace instead of a live LLM loop — the
        # no-key/no-network demo path that reproduces the recorded proposals deterministically.
        ...
        self.processed = 0
        self.total = 0
        self._llm = llm               # injectable (tests); else built lazily via make_llm()
        self.traces: dict[str, object] = {}   # agent_id -> LoopResult decision trace (loop mode)
    ...
    def _sourcing_agent_worker(self, agent_id: str, territory: dict):
        """Level 2: this agent sources its OWN territory — either as the autonomous LLM tool-use
        LOOP (agent_mode="loop", emitting a decision trace) or the fixed deterministic FLOW.

        The loop needs a real tool-calling LLM; with no key it falls back to the deterministic
        flow so the demo still runs offline. Either way each proposal is streamed to the Governor.
        """
        ...
        try:
            ...
            search_kw = {"search_fn": self.search_fn} if self.search_fn else {}

            ran_loop = False

            # Cached REPLAY path: reproduce a recorded decision trace with no key and no network.
            # Each agent replays its OWN trace (its own ReplayLLM), so there is no shared-LLM race
            # and no key is ever built — the reliable offline demo/video path.
            if self.agent_mode == "loop" and self.replay_traces is not None:
                trace = self.replay_traces.get(territory.get("key"))
                if trace is not None:
                    try:
                        from .loop_agent import replay_trace
                        result = replay_trace(trace, territory, on_proposal=on_proposal, **search_kw)
                        with self._lock:
                            self.traces[agent_id] = result
                        ran_loop = True
                        self._log(f"{agent_id} replay: {len(result.proposals)} proposals, "
                                  f"stopped on {result.terminal_reason}")
                    except Exception as e:
                        self._log(f"{agent_id} replay failed ({str(e)[:60]}), falling back to flow")

            llm = None
            if not ran_loop and self.use_llm:
                try:
                    llm = self._get_llm()
                except Exception as e:
                    self._log(f"{agent_id} no LLM ({str(e)[:60]}), using deterministic flow")

            if not ran_loop and self.agent_mode == "loop" and llm is not None:
                try:
                    from .loop_agent import run_loop_agent
                    result = run_loop_agent(territory, llm, use_llm=True,
                                            on_proposal=on_proposal, **search_kw)
                    with self._lock:
                        self.traces[agent_id] = result
                    ran_loop = True
                    self._log(f"{agent_id} loop: {len(result.proposals)} proposals, "
                              f"stopped on {result.terminal_reason}")
                except Exception as e:
                    # If the loop already emitted proposals before failing, the flow fallback
                    # below is (correctly) skipped to avoid duplicate proposals — so DON'T claim
                    # we fell back. Surface the partial state honestly instead.
                    if emitted["n"] > 0:
                        self._log(f"{agent_id} loop errored after {emitted['n']} proposal(s) "
                                  f"({str(e)[:50]}); keeping partial, no flow fallback "
                                  f"(would duplicate)")
                    else:
                        self._log(f"{agent_id} loop failed ({str(e)[:50]}), falling back to flow")

            # Deterministic flow: the offline/no-key fallback, or the default when not in loop mode.
            # Only run if the loop did not already emit this agent's proposals (avoid duplicates).
            if not ran_loop and emitted["n"] == 0:
                try:
                    run_sourcing_agent(territory, llm=llm, use_llm=self.use_llm,
                                       on_proposal=on_proposal, **search_kw)
                except Exception as e:
    ...
    def snapshot(self) -> dict:
        with self._lock:
            ...
            return {
                ...
                "traces": dict(self.traces),   # agent_id -> LoopResult (loop mode only)
            }


def run_headless(mode: str, source: str = "labeled", agent_mode: str = "flow",
                 use_llm: bool = False, search_fn=None, llm=None, replay_traces=None) -> dict:
    orc = Orchestrator(mode=mode, zero_mode="stub", step_delay=0.02, source=source,
                       agent_mode=agent_mode, use_llm=use_llm, search_fn=search_fn, llm=llm,
